    ...
    fn to_tokens(&self, output: &mut TokenStream2) {
        ...
        let register_stream = match args {
            JobArgs::OneShot(literal) => {
                quote! { __jobs.add_job(::autumn_job::job::Job::one_shot(#literal).run(#name))}
            }
            JobArgs::FixDelay(literal) => {
                quote! { __jobs.add_job(::autumn_job::job::Job::fix_delay(#literal).run(#name))}
            }
            JobArgs::FixRate(literal) => {
                quote! { __jobs.add_job(::autumn_job::job::Job::fix_rate(#literal).run(#name))}
            }
            JobArgs::Cron(literal) => {
                quote! { __jobs.add_job(::autumn_job::job::Job::cron(#literal).run(#name))}
            }
        };
        let stream = quote! {
            #(#doc_attributes)*
            ...
            impl ::autumn_job::handler::TypedHandler for #name {
                fn install_job(self, __jobs: &mut ::autumn_job::Jobs) -> &mut ::autumn_job::Jobs {
                    #ast
                    #register_stream
                }
            }
