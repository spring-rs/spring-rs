        ...
        impl ToTokens for Method {
            fn to_tokens(&self, output: &mut TokenStream2) {
                let stream = match self {
                    $(Self::$variant => quote!(::autumn_web::MethodFilter::$upper),)+
                };
...
impl ToTokens for Route {
    fn to_tokens(&self, output: &mut TokenStream2) {
        ...
        let registrations: TokenStream2 = args
            ...
            .map(|args| {
                ...
                let method_binder = methods
                    .into_iter()
                    .map(|m| quote! {let __method_router=::autumn_web::MethodRouter::on(__method_router, #m, #name);});

                quote! {
                    let __method_router = ::autumn_web::MethodRouter::new();
                    #(#method_binder)*
                    __router = ::autumn_web::Router::route(__router, #path, __method_router);
                }
            })
            .collect();
        ...
        let stream = quote! {
            ...
            impl ::autumn_web::handler::TypedHandler for #name {
                fn install_route(self, mut __router: ::autumn_web::Router) -> ::autumn_web::Router{
                    #ast
                    #registrations
